...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((array, nextVal) => {
    array.push(nextVal.name);
    return array;
  }, []);
};
...
const reversedString = (str) => {
  // Solution code here...
  return str.split('').reduce((firstChar, secChar) => secChar + firstChar);
};
...
const countNumberOfChildren = (arr) => {
  ...
  return arr.reduce((count, character) => {
    return character.children ? count += character.children.length : count;
  }, 0)
};
...
const calculateAverage = (arr) => {
  // Solution code here...
  let sumOfArr = arr.reduce((obj, number) => {
    obj.count++;
    obj.sum += number;
    return obj;
  } ,{ count: 0, sum: 0 })
  return sumOfArr.sum/sumOfArr.count;
};
