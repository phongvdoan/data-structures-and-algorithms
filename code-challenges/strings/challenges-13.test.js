...
const findAnything = (arr, target) => {
  // Solution code here...
  return arr.reduce((newArr, currentArr) => {
    currentArr.includes(target) ? newArr.push(currentArr) : newArr;
    return newArr;
  }, []);
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.reduce((outerArr, currentArr) => {
    let currentArrResult = currentArr.filter(string => !string.includes('Brook'));
    outerArr.push(currentArrResult);
    return outerArr;
  }, [])
};
